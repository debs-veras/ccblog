          ...
          render={({ field }) => {
            ...
            return (
              <Select
                ...
                isLoading={isLoading}
                isClearable={!isMulti}
                isMulti={isMulti} // <-- ativando múltipla seleção
...
export function InputQuill(props: InputQuillProps) {
  ...
  const handleImageUpload = useCallback(() => {
    ...
    input.onchange = async () => {
      ...
      const apiBase = (import.meta.env.VITE_URL_API || "").replace(/\/+$/, "");
      const imageUrl =
        response.data.url.startsWith("/") && apiBase
  ...
  const modules = useMemo(
    () => ({
      toolbar: {
        container: isMinimal ? minimalToolbar : fullToolbar,
        handlers: {
      ...
      <Controller
        ...
        render={({ field }) => (
          <QuillEditor
            ...
            style={{ minHeight: 240 }}
            value={field.value || ""}
            placeholder={placeholder || "Escreva seu texto aqui"}
